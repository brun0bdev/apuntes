...
async function doLogos() {
  await mkdir(path.join(ASSETS_DIR, "teams"), { recursive: true });
  const results = { ok: [], failed: [] };
  for (const { id, file } of TEAM_LOGOS) {
    const dest = path.join(ASSETS_DIR, "teams", `${id}.webp`);
    if (skipReason(dest)) {
    ...
    try {
      ...
      manifestSet(`teams/${id}.webp`, { source: `github:webBrunoB (lecVersus/img/logos/${file})`, url: `${GITHUB_RAW}/lecVersus/img/logos/${file}`, bytes });
...
async function doPlayerPhotos() {
  ...
  await mkdir(path.join(ASSETS_DIR, "players"), { recursive: true });
  ...
  for (const p of players) {
    ...
    const existing = exts.map((e) => path.join(ASSETS_DIR, "players", `${p.id}.${e}`)).find(existsSync);
    if (existing && !FORCE) {
      console.log(`players/${p.id}: ya existe (${path.basename(existing)}), se salta`);
      skipped++;
      continue;
    }
    if (manifest.assets[`players/${p.id}.png`]?.fallback && !FORCE) {
      console.log(`players/${p.id}: sin foto conocida (manifest), se salta`);
      skipped++;
      continue;
    }
    attempted++;
    ...
    try {
      ...
      const dest = path.join(ASSETS_DIR, "players", `${p.id}.${ext}`);
      const bytes = await downloadTo(info.imgSrc, dest);
      manifestSet(`players/${p.id}.${ext}`, { source: `fandom:${wiki} (infoboxPlayer)`, url: info.imgSrc, bytes });
      // limpiar restos de extensiones previas distintas
      for (const e of exts) {
        if (e !== ext) {
          const other = path.join(ASSETS_DIR, "players", `${p.id}.${e}`);
          if (existsSync(other)) (await import("node:fs/promises")).unlink(other).catch(() => {});
        }
      }
      downloaded++;
      console.log(`players/${p.id}.${ext}: OK (${bytes} bytes)`);
    } catch (err) {
      failed++;
      failDetail.push(`${p.id}: ${err.message}`);
      manifestSet(`players/${p.id}.png`, { source: "fandom", url: null, fallback: true, reason: err.message });
      console.warn(`players/${p.id}: FALLO — ${err.message}`);
    }
    await pause();
  }

...
export type SortKey = 'contractEnd' | 'name' | 'team' | 'role' | 'agent' | 'rating' | 'nationality';
...
export function sortPlayers(list: Player[], key: SortKey, dir: SortDir): Player[] {
  return [...list].sort((a, b) => {
    ...
    // rating es numérico: comparación aritmética, no alfabética.
    const comparison =
      typeof valueA === 'number' && typeof valueB === 'number'
        ? valueA - valueB
        : String(valueA).localeCompare(String(valueB), 'es', { sensitivity: 'base', numeric: true });

...
function initSimulator() {
    ...
    const quickReset = document.getElementById('quick-reset-btn');
    if (quickReset) {
        quickReset.addEventListener('click', resetSimulation);
    }

    const quickRandom = document.getElementById('quick-random-btn');
    if (quickRandom) {
        quickRandom.addEventListener('click', randomizeResults);
    }
}
...
function updateProgress() {
    ...
    updateUXProgress(decided, total);
...
function updateUXProgress(decided, total) {
    const compactFill = document.getElementById('xp-progress-fill');
    const compactCount = document.getElementById('xp-progress-count');
    const pct = total ? (decided / total) * 100 : 0;

    if (compactFill) compactFill.style.width = `${pct}%`;
    if (compactCount) compactCount.textContent = `${decided}/${total} partidos`;

    const simStep = document.getElementById('xp-step-sim');
    if (simStep) {
        simStep.classList.toggle('completed', decided > 0);
        const status = simStep.querySelector('.xp-step-status');
        if (status) status.textContent = decided > 0 ? 'En marcha' : 'Pendiente';
    }

    const completeStep = document.getElementById('xp-step-complete');
    if (completeStep) {
        const done = decided === total && total > 0;
        completeStep.classList.toggle('completed', done);
        const status = completeStep.querySelector('.xp-step-status');
        if (status) status.textContent = done ? 'Listo para compartir' : 'Opcional';
    }
}
...
function updateScenarioStep(teamId) {
    const scenarioStep = document.getElementById('xp-step-scenarios');
    if (!scenarioStep) return;

    const hasSelection = Boolean(teamId);
    scenarioStep.classList.toggle('completed', hasSelection);
    const status = scenarioStep.querySelector('.xp-step-status');
    if (status) status.textContent = hasSelection ? 'Analizando' : 'Elige un equipo';
}
...
function initTeamSelector() {
    ...
    select.addEventListener('change', (e) => {
        if (e.target.value) {
            updateScenarioStep(e.target.value);
    ...
    updateScenarioStep(select.value);
}
